...
def update_task(task_id: int, task: schemas.TaskUpdate, db: Session = Depends(get_db)):
    ...
    redis_cache.set(
        f"task:{task_id}", json.dumps(task_schema.model_dump()), ex=CACHE_TTL
    )
    delete_get_all_cache(redis_cache, "tasks")
    return task_schema
...
def partial_update_task(
    task_id: int, task: schemas.TaskPartialUpdate, db: Session = Depends(get_db)
):
...
def get_tasks(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    ...
    redis_cache.set(
        cache_key,
        json.dumps([task.model_dump() for task in task_schemas]),
        ex=CACHE_TTL,
    )
    return task_schemas
